...
| Status | 11 modules · 3,041 lines · **80 tests passing** · 15 endpoints |
...
- **No auth, accounts, or multi-tenancy.** One case per browser, one per phone number.
...
**Judge moment:** run `pytest -v` on the projector. 80 green. Do not explain the tests.
...
**The enforcement needed no new safety code.** `validate_citations()` builds its allowed
set from Tier A alone, so any web-derived citation the model emits is deleted by the
renderer that already existed.

> **On stage:** "We do search the web — government domains only. And the renderer still
> deletes every citation that isn't human-verified. Here's a draft where it stripped one."

`test_sources.py::test_web_citation_is_stripped_from_the_filing` asserts exactly this. If
it goes red, that sentence has become a lie. Release blocker.
